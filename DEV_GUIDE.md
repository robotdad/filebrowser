...
### CLI and service management: `filebrowser/cli.py` and `filebrowser/service.py`

filebrowser is now a uv-installable tool with built-in service management:

| Command | Purpose |
|---------|---------|
| `filebrowser serve` | Run the uvicorn server (default: 127.0.0.1:58080) |
| `filebrowser service install` | Install systemd user service + Caddy config snippet |
| `filebrowser service start/stop/restart` | Control the service |
| `filebrowser service status/logs` | View service status and logs |
| `filebrowser service uninstall` | Remove service and config |
| `filebrowser upgrade` | Update to latest version (uv tool upgrade + restart) |

### Deployment artifacts: `deploy/` (deprecated)

The shell-script deployment is deprecated in favor of `uv tool install`. Files remain for reference:

| File | Purpose |
|------|---------|
| `install.sh` | Legacy installer: FQDN detection, PAM setup, rsync to /opt, venv, Caddy install, systemd unit |
| `update.sh` | Legacy updater: rsync + pip upgrade + systemctl restart |
| `filebrowser.service` | Legacy systemd unit template (system-level service) |
| `filebrowser.caddy.template` | Caddy HTTPS config template (still used by `filebrowser service install`) |
| `filebrowser.caddy.http.template` | Caddy HTTP fallback template |

### Runtime config

| File | Location | Contents |
|------|----------|----------|
| `secret_key` | `~/.config/filebrowser/secret_key` | Persisted secret key for session signing (mode 0600) |
| Systemd unit | `~/.config/systemd/user/filebrowser.service` | User service (or `/etc/systemd/system/` for legacy) |
| Caddy snippet | `/etc/caddy/conf.d/filebrowser.caddy` | Generated by `filebrowser service install` |
...
```bash
filebrowser upgrade              # stops service, upgrades via uv, restarts
```
